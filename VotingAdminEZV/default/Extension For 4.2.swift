...
extension AppDelegate {
    ...
    func showAlert(title : String?, message : String?, actions:[UIAlertAction] = [UIAlertAction(title: "확인", style: .cancel, handler: nil)], viewCon : UIViewController? = nil, complete:(()->Void)? = nil){
        ...
        DispatchQueue.main.async {
            if let presentVC = viewCon ?? appDel.topVC {
                presentVC.present(self.alertCon!, animated: true, completion: {
                    complete?()
                })
            }
        }
    ...
    func euckrEncoding(_ query: String) -> String { //EUC-KR 인코딩
        ...
        let outputQuery = eucKRStringData.map {byte->String in
            if byte >= UInt8(ascii: "A") && byte <= UInt8(ascii: "Z")
                ...
                || byte == UInt8(ascii: "_") || byte == UInt8(ascii: ".") || byte == UInt8(ascii: "-") || byte == UInt8(ascii: "é") || byte == UInt8(ascii: "É")
            {
